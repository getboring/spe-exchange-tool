...
CREATE TABLE profiles (
  ...
  ebay_store_type TEXT DEFAULT 'basic' CHECK (ebay_store_type IN ('none', 'starter', 'basic', 'premium', 'anchor')),
...
CREATE TABLE items (
  ...
  name TEXT NOT NULL,
  platform TEXT,
  condition TEXT CHECK (condition IN ('loose', 'cib', 'sealed')),
  type TEXT DEFAULT 'game' CHECK (type IN ('game', 'console', 'accessory')),
  weight TEXT DEFAULT '8oz',
  status TEXT DEFAULT 'in_stock' CHECK (status IN ('in_stock', 'listed', 'sold')),
...
-- PRICE GUIDE (local cache of PriceCharting data)
...
CREATE POLICY "scans_user_access" ON scans
  FOR ALL USING (auth.uid() = user_id);

-- Price guide: all authenticated users can read
CREATE POLICY "price_guide_authenticated_read" ON price_guide
...
-- Storage bucket for item thumbnails (created via Supabase dashboard or CLI)
